...
func _load_random_layout() -> void:
	_load_layout(Level2Layouts.LAYOUTS[randi() % Level2Layouts.LAYOUTS.size()])
...
func _update_grid() -> void:
	for c in _gridbox.get_children():
		c.free()
	if not _grid_on:
		return
	for row in ROWS:
		for col in COLS:
			...
			box.size = Vector2(SLOT - 2, SLOT - 2)
			box.position = _center(cell) - Vector2(SLOT / 2.0 - 1, SLOT / 2.0 - 1)
			box.color = Color(0, 1, 1, 0.16)
			_gridbox.add_child(box)
